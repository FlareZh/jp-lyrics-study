...
- **读音可改**：用户可点击歌词振假名或浮层读音修改，按 `SONG_ID` 存本机；也可 Alt/⌥+点击振假名或在生词弹窗勾选「朗读时用振假名」强制按假名发音；设置里可「恢复默认读音」。生成时必须填写不重复的 `SONG_ID`。
- **日语发音**：模板用浏览器 **Web Speech API**（`ja-JP`）。自动优选常见高质量音色（如 Kyoko / O-ren / Google 日本語），并避开 macOS Eloquence 趣味音色；默认按汉字/词形朗读；用户改过的振假名，或手动指定「按假名发音」的词，才按假名读。顶部「全部朗读」可逐句连读，支持暂停/继续与停止。需系统/浏览器安装日语语音包。
