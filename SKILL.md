...
description: 生成"日语歌词逐句学习网页"。当用户给出日语歌曲名（可含歌手名/乐队名），要求"做歌词学习网页""逐句讲解歌词""生成日语学习HTML""日语歌词注音/分词/语法讲解页"时使用。流程：先检索官方歌词发给用户确认，确认后按词法/语法/翻译逐句拆解，生成含逐句讲解、可点击生词弹窗（词性/形态/读音/释义/出处）、按 JLPT 分级生词总表的自包含单文件 HTML。面向初学日语但要求精确语法讲解的学习者。歌词中的拉丁字母英文一律跳过、不做处理。
---

# 日语歌词学习网页

为指定日语歌曲生成一个**自包含单文件 HTML**（`歌词名-歌词学习.html`）：逐句显示歌词（带 `<ruby>` 振假名）、点击可展开该句的分词/语法/翻译，所有生词可点击弹出词性/形态/读音/释义/出处，底部生词总表按 JLPT 等级分组去重。歌词中的拉丁字母英文原样保留，不做任何学习标注。样式/交互/数据均内联；发音使用浏览器 Web Speech API（无需联网拉模型）。
...
- **展开箭头**：用内联 SVG，不用字符。
- **显示设置**：模板自带「设置」可开关振假名、片假名→平假名注音，并可调日语发音音色 / 语速 / 音调（自动优选或手动；localStorage）；生成页无需接线。片假名注音依赖数据里对片假名写了 `<ruby>…</ruby>`。
- **读音可改**：用户可点击歌词振假名修改，按 `SONG_ID` 存本机；也可 Alt/⌥+点击振假名或在生词弹窗勾选「朗读时用振假名」强制按假名发音；设置里可「恢复默认读音」。工具栏铅笔可整歌编辑 `[表记|读音]` 并应用预览，或复制文本交给 AI 按本 skill 重新生成（无导入；片假名不注平假名）。生成时必须填写不重复的 `SONG_ID`。
- **日语发音**：模板用浏览器 **Web Speech API**（`ja-JP`）。自动优选常见高质量音色（如 Kyoko / O-ren / Google 日本語），并避开 macOS Eloquence 趣味音色；默认按汉字/词形朗读；用户改过的振假名，或手动指定「按假名发音」的词，才按假名读。顶部「全部朗读」可从当前选中句逐句连读，再点同一按钮停止。需系统/浏览器安装日语语音包。
