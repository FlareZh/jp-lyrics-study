...
定级规则：
1. 优先用**词形**作 `k`（词典形，如 `浮かぶ` 而非 `浮かんで`）。
...
- **生词总表**：词性与形态放在生词按钮内部；按 JLPT 分组去重（无等级词归入「未分级」）。
- **hero 氛围**：每首歌按歌词意象同时定制 hero 背景配色与动画特效（如 夜蓝泡沫 / 深夜→破晓微光+飘雪 / 花瓣 / 星光…），配色与特效都要贴合本首歌情绪，不要沿用上一首。
- **展开箭头**：用内联 SVG，不用字符。
- **日语发音**：模板已用 Web Speech API（`speechSynthesis`，`ja-JP`）为每句歌词、生词、语法提供喇叭按钮；生成页无需额外接线。单词优先读假名读音，语法优先读 `<code>` 内句型。
