...
面向初学日语、但希望语法讲解尽量精确的学习者。输入歌名（可含歌手），确认官方歌词后，输出一个 HTML 文件：振假名、分词、语法、翻译、可点击生词浮层、piper-plus 日语发音（OpenJTalk，汉字读音准确），以及按 JLPT 分级的生词总表。
...
- 设置：可开关振假名、片假名的平假名注音；可改读音并本机保存（按歌曲）
...
3. 交付 `歌词名-歌词学习.html`（单文件；建议用本地 `http://` 打开）
